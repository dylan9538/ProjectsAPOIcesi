...
public class PanelVentaBebidaUno extends JPanel implements ActionListener {
	...
	public PanelVentaBebidaUno(InterfazBebidas v) {
		principal = v;

		TitledBorder border = BorderFactory
				.createTitledBorder("venta de Naranja-Limón");
		border.setTitleColor(Color.BLACK);
		setBorder(border);

		setLayout(new BorderLayout());

		ImageIcon icono = new ImageIcon("./img/bebidaUno.jpg");
		imagen = new JLabel("");
		...
		txtCosto = new JTextField(
				String.valueOf(VentaDeBebidas.PRECIO_POR_DEFECTO_BEBIDA_UNO));
		grid.add(labCosto);
		grid.add(txtCosto);
	...
	public void actionPerformed(ActionEvent evento) {
		...
		if (comando.equals(VASO)) {
			try {
				principal.gestionarVenta1VasoBebidaUno();
		...
		if (comando.equals(VASOMAS)) {
			try {
				principal.gestionarVentaVariosVasosBebidaUno();
			} catch (NumberFormatException e) {
				JOptionPane
						.showMessageDialog(
								this,
