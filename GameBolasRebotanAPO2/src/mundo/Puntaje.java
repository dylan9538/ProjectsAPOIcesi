...
public class Puntaje implements Serializable{
	...
	public final static int MAXIMO_LISTADO = 10;
	private String[][] nombres;
	private int[][] puntajes;
	
	public Puntaje(){
		nombres  = new String[NIVELES][MAXIMO_LISTADO];
		puntajes = new int[NIVELES][MAXIMO_LISTADO];
	...
	public boolean agregarPuntaje(int niv, String n, int p){
		...
		if(agrego){
			for(int j=MAXIMO_LISTADO-1;j>i;j--){
	...
	public String darPuntajes(){
		...
		reporte += "\n";
		for(int i=0;i<MAXIMO_LISTADO;i++){
			for(int nivel=0;nivel<nombres.length;nivel++){
	...
	public boolean entreLosMejores(int niv, int p){
		...
		if(p<puntajes[niv][MAXIMO_LISTADO-1]){
			entreMejores = true;
		}
		return entreMejores;
	}
